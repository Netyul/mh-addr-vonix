...
const onGenPDF = (params: string[]) => {
  ...
  desserts.value.forEach(async (param: string) => {
    ...
    let d = await Object.values(param)
    dados.push(d)
  })
...
<template>
      ...
      <VCard>
        <VCardTitle>
          <VRow>
            ...
            <VCol cols="6" class="text--secondary">
              <div class="d-flex flex-row-reverse" v-if="!search">
                <VBtn
                  size="small"
                  variant="outlined"
                  class="ml-2"
        ...
        <VCardText>
          ...
          <VDataTable
            ...
            class="elevation-1"
          >
            <template v-slot:item.date_login="{ item }">
              <v-chip >
  ...
  <!-- <vue3-html2pdf>
    <template  v-slot:pdf-content>
      este é o pdf gerado pelo Jefte
    </template>
  </vue3-html2pdf> -->
</template>
